...
async function cartForEdit(cart: CartRow) {
  ...
  const { data: sourceItems, error: sourceError } = await supabase
    .from("cart_items")
    .select("product_id,variant_id,quantity,add_ons")
    .eq("cart_id", cart.id);
  ...
  if (sourceItems.length) {
    const { error: copyError } = await supabase
      .from("cart_items")
      .insert(sourceItems.map((item) => ({ ...item, cart_id: id })));
    if (copyError) {
...
async function cartDto(cart: CartRow): Promise<CartDto> {
  ...
  const { data, error } = await supabase
    .from("cart_items")
    .select("id,product_id,variant_id,quantity,add_ons")
    .eq("cart_id", cart.id)
    .order("created_at");
  if (error) throw error;
  ...
  const items = ((data ?? []) as ItemRow[]).map((row) => {
    ...
    if (!catalog)
      ...
      warnings.push({
        code: "STOCK_CHANGED",
        itemId: row.id,
        message: `Only ${catalog.variant.stock} are currently available.`,
      });
    const unitPrice = catalog ? effectiveUnitPrice(toMinorUnits(catalog.variant.price)) : 0;
        ...
        ? [
            {
              ...
              unitPrice: effectiveUnitPrice(toMinorUnits(addOn.price)),
              quantity: Math.min(saved.quantity, row.quantity),
            },
          ]
        : [];
    });
    const lineTotal =
      ...
      addOns.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0);
    return {
      itemId: row.id,
      productId: row.product_id,
      variantId: row.variant_id,
      slug: catalog?.product.slug ?? "",
      name: catalog?.product.name ?? "Unavailable product",
      variantName: catalog?.variant.color ?? "",
      imageUrl: catalog?.variant.collectionImage ?? null,
      unitPrice,
      quantity: row.quantity,
      lineTotal,
      available,
      maxQuantity: catalog ? Math.min(catalog.variant.stock, MAX_CART_QUANTITY) : 0,
      addOns,
    };
  });
  return {
...
export async function addCartItem(input: {
  ...
  if (catalog.variant.stock < input.quantity)
    throw new CartError("OUT_OF_STOCK", "The requested quantity is not available.", 409);
  const addOns = (input.addOns ?? [])
    .filter((item) => findAddOn(item.id))
    .map((item) => ({
      id: item.id,
      quantity: Math.min(input.quantity, Math.max(1, item.quantity)),
    }));
...
export async function updateCartItem(
  ...
  if (!catalog || catalog.variant.stock < quantity)
    throw new CartError("OUT_OF_STOCK", "The requested quantity is not available.", 409);
  const addOns = (requestedAddOns ?? item.add_ons)
  ...
  const { error } = await supabase
    .from("cart_items")
    .update({ quantity, add_ons: addOns, updated_at: new Date().toISOString() })
...
export async function validateCartForCheckout() {
  ...
  if (cart.warnings.length || cart.items.some((item) => !item.available))
    throw new CartError("CART_CONFLICT", "Your cart has items that need attention.", 409);
  return cart;
}
