  ...
  return (
    <ShopLayout>
      <div className="max-w-[1280px] mx-auto px-6 py-14">
        <h1 className="text-4xl font-semibold mb-3" style={{ color: "var(--color-text-primary)" }}>
          About FastHaus
        </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          {philosophy.map((p) => (
            <div key={p.label}>
              <span className="text-2xl mb-3 block" style={{ color: "var(--color-accent-amber)" }}>{p.icon}</span>
              <h3 className="text-sm font-semibold mb-1" style={{ color: "var(--color-text-primary)" }}>{p.label}</h3>
              <p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>{p.desc}</p>
            </div>
          ))}
        </div>
