        ...
        >
          ...
          <div className="flex flex-col">
            {(items ?? []).map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between gap-5 border-t py-4"
                style={{ borderColor: "var(--color-border)" }}
              >
                <div>
                  <p className="type-title-sm">{item.product_name}</p>
          ...
          >
            ...
            <DirhamPrice amount={Number(order.total)} size="lg" />
          </div>
